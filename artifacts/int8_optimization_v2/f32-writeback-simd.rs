...
fn matmul(activation: &[i8], scales: &[f32], weight: &Int8Matrix, tokens: usize) -> Vec<f32> {
    ...
    while t < padded_tokens {
        ...
        if MAX_TILE_ROWS >= 16 && remaining >= 16 {
...
/// Diagnostic-only 128-token kernel: time the integer dots and F32 writeback
/// separately. Its extra counter calls change code generation, so compare its
/// total with `benchmark` before interpreting either component.
pub fn benchmark_components(
    ...
    let start = counter();
    for t in (0..TOKENS).step_by(16) {
        let a = &activation[t * cols..(t + 16) * cols];
        for r in (0..rows).step_by(16) {
            let before = counter();
            let sums = dot_tile::<16, 16>(a, &weight.values[r * cols..(r + 16) * cols], cols);
            dots += counter().saturating_sub(before);
            let before = counter();
            for i in 0..16 {
                for j in 0..16 {
                    out[(t + i) * rows + r + j] =
                        sums[i][j] as f32 * scales[t + i] * weight.scales[r + j];
                }
            }

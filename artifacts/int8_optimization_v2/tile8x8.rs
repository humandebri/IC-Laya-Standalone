...
impl Int8Matrix {
    ...
    pub fn forward(&self, x: &Tensor) -> candle_core::Result<Tensor> {
        ...
        let out = crate::profile::measure("int8.matmul", shape, || matmul(&activation,&scales,self,tokens));
...
const MAX_TILE_COLS:usize=8;
const PAD_TAIL:bool=false;

fn matmul(activation:&[i8],scales:&[f32],weight:&Int8Matrix,tokens:usize)->Vec<f32>{
    let cols=weight.cols;
    let padded_tokens=if PAD_TAIL && tokens>2{tokens.div_ceil(4)*4}else{tokens};
    let padded;
    let (a,scales)=if padded_tokens!=tokens{
        let mut input=activation.to_vec();input.resize(padded_tokens*cols,0);
        let mut sx=scales.to_vec();sx.resize(padded_tokens,1.);
        padded=(input,sx);(&padded.0[..],&padded.1[..])
    }else{(activation,scales)};
    let mut out=vec![0f32;padded_tokens*weight.rows];
    let mut t=0;
    while t<padded_tokens{
        let remaining=padded_tokens-t;
        if MAX_TILE_ROWS>=16 && remaining>=16{write_tile::<16>(a,scales,weight,t,&mut out);t+=16;}
        else if MAX_TILE_ROWS>=8 && remaining>=8{write_tile::<8>(a,scales,weight,t,&mut out);t+=8;}
        else if remaining>=4{write_tile::<4>(a,scales,weight,t,&mut out);t+=4;}
        else if remaining>=2{write_tile::<2>(a,scales,weight,t,&mut out);t+=2;}
        else{write_tile::<1>(a,scales,weight,t,&mut out);t+=1;}
    }
    out.truncate(tokens*weight.rows);out
}
fn write_tile<const R:usize>(a:&[i8],sx:&[f32],w:&Int8Matrix,t:usize,out:&mut[f32]){
    let a=&a[t*w.cols..(t+R)*w.cols];
    let mut r=0;
    while r<w.rows{
        macro_rules! write {($c:literal)=>{{
            let sums=dot_tile::<R,$c>(a,&w.values[r*w.cols..(r+$c)*w.cols],w.cols);
            for i in 0..R{for j in 0..$c{out[(t+i)*w.rows+r+j]=sums[i][j] as f32*sx[t+i]*w.scales[r+j];}}
            r+=$c;
        }}}
        if MAX_TILE_COLS>=16 && r+16<=w.rows{write!(16);}
        else if MAX_TILE_COLS>=8 && r+8<=w.rows{write!(8);}
        else if r+4<=w.rows{write!(4);}
        else{write!(1);}
    }
}
...
fn dot_tile<const R:usize,const C:usize>(a:&[i8],b:&[i8],cols:usize)->[[i32;C];R]{
    ...
    for i in 0..R{for j in 0..C{for k in 0..cols{result[i][j]+=a[i*cols+k] as i32*b[j*cols+k] as i32;}}}
    result
}
#[cfg(target_arch="wasm32")]
#[allow(unsafe_code)]
fn dot_tile<const R:usize,const C:usize>(a:&[i8],b:&[i8],cols:usize)->[[i32;C];R]{
    assert!(R<=16 && C<=16 && cols<=16384 && a.len()==R*cols && b.len()==C*cols);
    use std::arch::wasm32::*;
    ...
    unsafe fn kernel<const R:usize,const C:usize>(a:&[i8],b:&[i8],cols:usize)->[[i32;C];R]{
        ...
        let mut k=0;
        while k<end{
            let mut lo=[i32x4_splat(0);C];let mut hi=[i32x4_splat(0);C];
        ...
        for i in 0..R{for j in 0..C{for k in end..cols{result[i][j]+=a[i*cols+k] as i32*b[j*cols+k] as i32;}}}
        result
    }
...
pub fn benchmark(tokens:usize,rows:usize,cols:usize,counter:fn()->u64)->candle_core::Result<(u64,[u8;32],Vec<crate::profile::Cost>)>{
    if !(1..=128).contains(&tokens) || ![1024,3072,5248].contains(&rows) || ![1024,2624].contains(&cols){candle_core::bail!("benchmark shape")}
    let mut bytes:Vec<u8>=(0..rows*cols).map(|i|((i*37+11)%256) as u8).collect();
    for _ in 0..rows{bytes.extend(0.25f32.to_le_bytes());}
    let w=Int8Matrix::from_bytes(&bytes,rows,cols)?;
    let values:Vec<f32>=(0..tokens*cols).map(|i|((i*19%255) as i32-127) as f32).collect();
